...
public class actNewOfferTime extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.act_new_offer_time);
        Toolbar toolbar =  findViewById(R.id.toolbar);
        setSupportActionBar(toolbar);
        builder = new AlertDialog.Builder(context);
        appExit.act2 = context;
...
//        for(int i=spinService1.getSelectedItemPosition(); i>=0; i--){
//            String alaki = spinService1.getItemAtPosition(i).toString();
//            if(spinService1.getItemAtPosition(i).toString().substring(alaki.length() - 30) == "     ____________________     "){
//                String title = spinService1.getItemAtPosition(i).toString().replace("____________________", "").trim();
//                String asw = "ASD";
//                break;
//            }
//        }
...
////                    "', '" + txtSkillPrice.getText().toString().trim() + "')";
            Query = "INSERT INTO TB_OfferTime (id, subservice_id_1, subservice_id_2, discount_amount) VALUES ('" +
                    ...
                    "', '" + txtOfferPrice.getText().toString().trim() + "')";

            SQL.Execute(Query);
            Snackbar name = Snackbar.make(coordiantLayout, "تخفیف انتخاب اضافه شد", Snackbar.LENGTH_LONG);
            ...
            Query = "UPDATE TB_OfferTime SET subservice_id_1='" + Integer.toString(lSubService1.get(0).id()).trim() +
        ...
        Data.put("subservicesdicsount_id", mID);
        Data.put("subservicesdicsount_subservice_id_1", adapter.getItemString(spinService1.getSelectedItemPosition(), "id"));
        Data.put("subservicesdicsount_subservice_id_2", adapter.getItemString(spinService2.getSelectedItemPosition(), "id"));
        Data.put("subservicesdicsount_discount_amount", txtOfferPrice.getText().toString().replace(",", ""));
    ...
    void Undoo(){
        String Query = "DELETE FROM TB_OfferTime WHERE id='" + mID + "'";
        SQL.Execute(Query);
        Snackbar name = Snackbar.make(coordiantLayout, "تخفیف هم زمانی حذف شد", Snackbar.LENGTH_LONG);
        name.show();
