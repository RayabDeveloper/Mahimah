...
import net.rayab.mahimah.Adapter.Customers.OnCustomerListChangedListenerOffer;
...
public class adapOfferChoise extends RecyclerView.Adapter<adapOfferChoise.AdapterMember>{

    private Context context;
    private List<Datas.SubServicesDiscount> lList;
    private AlertDialog.Builder builder;

    public adapOfferChoise(List<Datas.SubServicesDiscount> lList, Context context){
        this.lList = lList;
    ...
    public void onBindViewHolder(final AdapterMember holder, @SuppressLint("RecyclerView") final int position) {
            ...
            public void onClick(View v) {
                ...
                builder.setPositiveButton("YES", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        Data.put("subservicesdicsount_id2", Integer.toString(lList.get(position).id()));

                        wAPI.API("https://mahimah.com/app/Deleter.php", Data);

                        String Query = "DELETE FROM TB_OfferChoise WHERE id='" + Integer.toString(lList.get(position).id()) + "'";
                        MySQLi SQL = new MySQLi(context);
        ...
        cardView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                actNewOfferChoise.mID = Integer.toString(lList.get(position).id());
                actNewOfferChoise.Service1 = lList.get(position).subservice_id_1();
                actNewOfferChoise.Service2 = lList.get(position).subservice_id_2();
                actNewOfferChoise.Amount = lList.get(position).discount_amount();
                actNewOfferChoise.Type = 2;
                Intent intent = new Intent(context, actNewOfferChoise.class);
